...
board en geen leaphymicropython nodig. Draai eerst `pnpm lego:extract`.
...
EXTRACTED = Path(__file__).resolve().parents[2] / "lego-tests" / "extracted"


def main() -> int:
    index_pad = EXTRACTED / "index.json"
    if not index_pad.exists():
        print("geen lego-tests/extracted/index.json - draai eerst `pnpm lego:extract`")
        return 1

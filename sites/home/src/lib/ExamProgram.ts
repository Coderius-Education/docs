...
export function formatKeuzeDomainTooltip(letter: string): string {
  ...
  const subText = subs.map((d) => `${d.code} ${d.name}\n${d.description}`).join('\n\n');
...
export function buildCardChips(mappings: ExamMapping[] | undefined): CardChip[] {
  ...
  const keuzeChips: CardChip[] = Array.from(keuzeGroups.entries()).map(([letter, ms]) => {
    const strength: 'strong' | 'weak' = ms.some((m) => m.strength === 'strong') ? 'strong' : 'weak';
    const group = keuzeDomainGroupByLetter.get(letter);
    const header = group ? `${group.letter} — ${group.name}` : letter;

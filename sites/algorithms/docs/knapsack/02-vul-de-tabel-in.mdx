...
Je bouwt een tabel op van klein naar groot. Je begint met "0 items in
de rugzak" (rij 0) en vult rij voor rij door tot "alle 5 items
beschikbaar".

Voor elke cel `(i, w)` beantwoord je dezelfde vraag: *"als ik items
1 t/m `i` mag gebruiken en mijn rugzak is `w`, wat is dan de beste
totale waarde?"*

Bij het invullen van een cel kijk je alleen naar cellen die al ingevuld
zijn, in de rij erboven. Geen recursie en geen dubbel rekenwerk: één
tabel die zichzelf opbouwt.

## Het algoritme in drie regels

Elke cel `OPT(i, w)` is "de beste totaalwaarde als je items 1 t/m i
...
Teken hem op papier, of print de [hand-out](/docs/unplugged/05-pak-de-rugzak-in)
met een kleinere tabel om in te vullen. We vullen hem hier rij voor rij.
...
Daarvoor loop je de tabel terug vanaf rechtsonder.
...
In de tabel kun je dat volgen: een kopieer-cel (regel 2, of skip wint
in regel 3) wijst naar boven, een meenemen-cel wijst schuin naar
linksboven.
...
In de volgende stappen schrijf je precies deze tabel in Python: eerst
de items, dan de lege tabel, dan rij voor rij invullen, zoals je het
hier met de hand deed.
...
Geen recursie en geen kopieën: één tabel, die je zelf al hebt
uitgerekend.

:::info
Een kleinere tabel klassikaal invullen, met een gok vooraf? Print [Pak de rugzak in](/docs/unplugged/05-pak-de-rugzak-in): items, regels en tabel op één blad.
:::

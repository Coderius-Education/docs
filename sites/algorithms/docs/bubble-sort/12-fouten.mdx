...
**Oorzaak:** bij de laatste ronde is `i = n - 1`, dus `lijst[i + 1]` is
`lijst[n]`, en die bestaat niet. Vandaar de `IndexError`.
...
trouwens niet `==` maar `>`, want je wilt weten of het paar fout staat.
...
Geen foutmelding, maar de early-exit doet niet meer wat hij moet doen.

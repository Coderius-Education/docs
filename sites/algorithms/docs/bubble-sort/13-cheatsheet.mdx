...
{/* foutmelding-los: samenvatting van de foutenpagina, waar beide meldingen gereproduceerd worden */}

1. `range(len(lijst))` in binnenste lus zonder `- 1` → `IndexError`.
2. `=` in `if` waar je `==` of `>` bedoelt → `SyntaxError`.
3. `geswapt` één keer op `False` vóór de buitenste lus → de vlag wordt niet
   gereset → early-exit werkt verkeerd.
...
Sneller sorteren dan `O(n²)` kan met merge sort of quicksort. Die staan
niet in deze cursus, maar [Big O](/docs/big-o/06-kwadratisch) laat zien
hoeveel het scheelt.

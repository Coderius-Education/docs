...
Een tic-tac-toe-bord is 3 bij 3. In Python wordt dat een lijst van
drie lijsten, waarbij elke binnenste lijst één rij is.
...
CS50 noemt deze `X`, `O` en `EMPTY`, als constanten. Wij gebruiken de
strings `"X"`, `"O"` en de waarde `None` direct; dat is hetzelfde,
alleen zonder de constanten.

## Een zet = een tuple

Een **zet** is een coördinaat: speel op rij i, kolom j. Dat schrijven we
als een tuple:
...
Een tuple is een rijtje dat niet meer verandert. `zet[0]` is `2`,
`zet[1]` is `1`.
...
Dit doe je straks in bijna elke functie: twee geneste lussen, één voor
de rijen en één voor de kolommen.
...
In de bouwstenen hierna bouw je één voor één deze functies:
...
| `max_value(bord)`, `min_value(bord)` | wat is een positie waard als X (of O) aan zet is? |
| `minimax(bord)` | de AI: welke zet is optimaal? |

Eerst nog één pagina over hoe deze functies samenwerken.

Door naar [functies werken samen →](./05-functies-samenwerken).

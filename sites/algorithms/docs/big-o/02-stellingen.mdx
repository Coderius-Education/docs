...
voordat je de complexiteitsklassen één voor één bekijkt.
...
**Niet waar.** Voor kleine `n` kan het andersom zijn.
...
> *"Als `n` groot genoeg wordt, is een `O(log n)`-algoritme sneller dan een `O(n)`-algoritme."*
...
Door naar [O(1) — constante tijd →](./03-constant).

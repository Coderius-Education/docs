...
Voor een index heb je een geheel getal nodig, dus `//`.
...
- De elementen moeten met `<` en `==` te vergelijken zijn.
...
- Beste geval: het doel ligt meteen op het midden, dus één vergelijking.
- Slechtste geval: `⌊log₂(n)⌋ + 1` vergelijkingen.
- Dat noemen we `O(log n)`.
...
2. `laag < hoog` in de while waar `laag <= hoog` hoort → het laatste element wordt overgeslagen.

...
Welkom bij SQL Injection. Door slimme tekst in een invoerveld te typen kun je de database-opdracht erachter veranderen — en zo gegevens opvragen die niet voor jou bedoeld zijn.
...
Je ziet een rode SQL-foutmelding, zoiets als:
`SQL-fout: unrecognized token: "'1''"`
...
3. De database struikelt over het losse aanhalingsteken, dus jouw invoer beïnvloedt echt de query-syntax.
...
Als je een lege pagina krijgt, probeer dan: `1' OR '1'='1' -- ` (twee streepjes en een spatie) — dit lab draait op SQLite, waar `-- ` de rest van de regel wegcommentarieert. Het MySQL-hekje `#` werkt hier niet.

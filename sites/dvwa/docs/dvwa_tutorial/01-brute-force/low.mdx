...
Voordat je een tool inzet, doe je de aanval één keer met de hand. Ga terug naar het lab hierboven en probeer bij gebruikersnaam `admin` een paar veelgebruikte wachtwoorden: `admin`, `123456`, `password`. Let op de melding onder het formulier: bij een fout wachtwoord meldt de server dat gebruikersnaam of wachtwoord onjuist is; bij `password` verschijnt in plaats daarvan een welkomstmelding.

Met de hand duizenden wachtwoorden proberen is ondoenlijk. Daarom automatiseer je het met **Hydra**: dat programma stuurt in hoog tempo verzoek na verzoek, elk met een ander wachtwoord uit een woordenlijst. Op een echte DVWA-server (Engelstalig) herkent Hydra een mislukte poging aan het woord `incorrect` in het antwoord, en stopt zodra dat wegblijft.

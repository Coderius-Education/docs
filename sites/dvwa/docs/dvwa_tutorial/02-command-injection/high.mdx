...
Vul in: `127.0.0.1|ls` (zonder spatie achter de pipe). Doordat de exact match in het filter niet meer klopt, laat het filter het commando er gewoon doorheen glippen.

Het resultaat van een succesvolle poging zie je hieronder:

![Bestandslijst van de server na het omzeilen van het high-filter](high_success.png)

...
Je wilt `/etc/passwd` uitlezen en probeert `?page=/etc/passwd`. Resultaat: `ERROR: File not found.` Terwijl je toch het volledige pad opgeeft.

Het filter eist dat je invoer begint met het woord `file` (of exact `include.php` is). Een pad als `/etc/passwd` begint met een `/` en valt dus meteen af — net als `../../etc/passwd`. De whitelist doet hier zijn werk, zoals bedoeld.

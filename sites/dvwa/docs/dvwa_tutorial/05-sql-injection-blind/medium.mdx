...
Je verwacht een lijst met gegevens, zoals bij gewone SQL-injectie, maar die komt niet. Dat hoort zo: dit is *blinde* injectie. De pagina toont nooit data; je enige signaal is de tijd. Kijk dus naar de laadduur, niet naar de inhoud.

Krijg je geen vertraging terwijl je payload klopt? Dan koos je waarschijnlijk een ID dat geen rij oplevert. `SLEEP(5)` draait per gevonden rij: `1 AND SLEEP(5)` slaapt alleen als ID 1 bestaat. Bij een niet-bestaand ID vindt de database niets, en dus slaapt er ook niets.

...
> **Labbeperking:** Deze aanval draait om het aanpassen van een cookie via F12 (**Application** → **Cookies**). Dat werkt maar beperkt binnen het afgeschermde iframe van het lab. De stappen hieronder laten zien hoe de aanval werkt; om hem echt uit te voeren heb je een [lokale DVWA-installatie](/docs/dvwa_installatie) nodig.
...
Je verandert de cookie-waarde in F12 → **Application** → **Cookies** naar `1' AND SLEEP(5) #`, wacht... en de pagina reageert nog steeds direct. Geen vertraging van vijf seconden.

Het aanpassen van een cookie doet op zichzelf niets. De server leest de cookie pas als er een nieuw verzoek naar toe gaat. Zolang je alleen de waarde in het tabblad wijzigt maar de pagina niet opnieuw laadt of het formulier niet opnieuw verstuurt, blijft de server je oude verzoek gebruiken en voert hij je payload nooit uit.

Wijzig dus de cookie én trigger daarna een nieuw verzoek (herlaad de pagina of druk opnieuw op Submit). Reageert de pagina dan vijf seconden trager, dan heb je bewezen dat de database jouw `SLEEP(5)` uitvoert — ook via dit indirecte kanaal.

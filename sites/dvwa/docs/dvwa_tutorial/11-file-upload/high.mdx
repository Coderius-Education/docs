...
> **Labbeperking:** Deze aanval vereist geavanceerde tools zoals **exiftool** en een combinatie met de **File Inclusion**-kwetsbaarheid. Het lab simuleert de server-side logica, maar voor de volledige hands-on ervaring heb je een [lokale DVWA-installatie](/docs/dvwa_installatie) nodig.
...
Op het vorige level was de Content-Type header aanpassen genoeg. Hier hernoem je `shell.php` naar `shell.jpg`, uploadt hem, en de server weigert: `Dit is geen geldige afbeelding!`

De naam veranderen verandert de *inhoud* niet. `getimagesize()` kijkt niet naar de extensie maar naar de eerste bytes van het bestand — de zogeheten Magic Bytes die elke echte JPG of PNG als "handtekening" bovenaan heeft. Jouw hernoemde PHP-script begint met `<?php`, niet met een image-signature, dus de controle faalt.

Je hebt dus een bestand nodig dat *echt* een geldige afbeelding is én jouw code bevat. Dat maak je door PHP-code in de metadata van een bestaande foto te verstoppen (bijvoorbeeld met `exiftool`). De Magic Bytes kloppen dan, de extensie is `.jpg`, en je code reist mee — al heb je daarna nog een File Inclusion-kwetsbaarheid nodig om hem uit te voeren.

...
> **Labbeperking:** Deze aanval vraagt dat je JavaScript uitschakelt en de HTML aanpast — dat lukt niet binnen het afgeschermde iframe van het lab. De stappen hieronder laten zien hoe de aanval werkt; om hem echt uit te voeren heb je een [lokale DVWA-installatie](/docs/dvwa_installatie) nodig.
...
Je past met F12 de `value` van de dropdown aan naar `1`, klikt op Submit, en er springt een pop-up op: `Je mag dit account niet bekijken!` Het verzoek wordt niet verstuurd.

Dit is precies de valkuil van dit level. De `value` veranderen is niet genoeg, want het `onsubmit`-script draait nog steeds en vergelijkt jouw keuze met je eigen ID vóórdat het formulier vertrekt. Zolang JavaScript aanstaat, blokkeert die controle je.

De volgorde is dus andersom dan je zou denken: schakel **eerst** JavaScript uit (via je browserinstellingen of een extensie), pas **daarna** de `value` aan, en verstuur dan. Zonder actieve JavaScript is er niets meer dat de submit tegenhoudt — de server zelf controleert immers niks.

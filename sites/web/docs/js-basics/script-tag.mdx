...
Staat de `<script>`-tag in de `<head>` en roept de code een element direct aan (niet via een knop), dan bestaat de `<p>` nog niet: `getElementById` geeft `null` terug. Zet de `<script>`-tag altijd **onderaan** de `<body>`, na alle HTML-elementen. In [script.js](/docs/js-basics/scriptjs) zie je hoe je dit met `defer` ook vanuit de `<head>` oplost.

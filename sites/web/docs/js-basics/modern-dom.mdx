...
<body>
  ...
  <button onclick="markeerKlaar()">Klaar</button>
  <button onclick="markeerBelangrijk()">Belangrijk</button>
</body>
</html>`;
...
Een aparte functie met `classList.remove('actief')`, aangeroepen door een eigen knop.
...
HTML:
```html
<button onclick="nachtmodus()">Nachtmodus</button>
```
...
De knoppen roepen `markeerKlaar()` en `markeerBelangrijk()` aan. Doorstrepen doe je in CSS met `text-decoration: line-through;`. De twee klassen mogen tegelijk aan staan.
...
[← Beslissingen met if/else](/docs/js-basics/if-else) | [Events →](/docs/js-basics/events)

...
- Zet per ongeluk haakjes achter de functienaam in een listener (`muisErop()`). Wat gebeurt er bij het laden, en wat bij het bewegen?
...
Bouw het gedrag van de speelkaart: bij `mouseover` krijgt de kaart de klasse `actief` (die staat al in de CSS), bij `mouseout` raakt hij hem weer kwijt, en elke `click` telt mee in de `<p id="teller">`. Schrijf drie benoemde functies en koppel elk aan zijn event.
...
Drie functies, drie listeners op hetzelfde element (`getElementById('kaart')`). Voor de klasse gebruik je `classList.add` en `classList.remove` uit [Modern DOM](/docs/js-basics/modern-dom).

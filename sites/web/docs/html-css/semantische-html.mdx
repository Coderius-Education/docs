...
Tags als `<header>` en `<nav>` groeperen niet alleen, ze vertellen ook *wat* een blok is. Dat heet **semantische** HTML.

## Theorie

Tot nu toe heb je vooral `<div>` gebruikt om elementen te groeperen. Een `<div>` zegt niks over *wat* een blok is. Het is een doos, meer niet.
...
Visueel is er **geen verschil**: allebei tonen ze "Hoofdmenu". Voor screenreaders en zoekmachines wel: die herkennen `<nav>` als de navigatie, terwijl een `<div>` ze niets vertelt.

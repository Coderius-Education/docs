...
Een `<li>` moet binnen een `<ul>` of `<ol>` staan. Verraderlijk is dat het er goed uitziet: de browser tekent gewoon een bolletje, alleen zonder de inspringing die de lijst eromheen geeft. Screenreaders en zoekmachines zien dan geen lijst, en zodra je de lijst later opmaakt met CSS valt dat ene item buiten de boot.

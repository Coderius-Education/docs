...
export const startCss = `.highlight {
...
Een gewone `<a>` is `inline`. `padding` lijkt te werken — er komt ruimte rond de tekst — maar die ruimte duwt de regels erboven en eronder niet weg, dus twee van zulke links op opeenvolgende regels overlappen elkaar. Ook `width` en `height` doen niets. De oplossing staat in deze les:

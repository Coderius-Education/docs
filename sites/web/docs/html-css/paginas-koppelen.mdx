...
:::info
In de preview hieronder werken links: een link zonder `target` opent in het voorbeeldvenster zelf, een link mét `target="_blank"` opent een echt nieuw tabblad. Sommige sites weigeren zich in een klein venster te laten tonen; dan blijft het voorbeeld leeg en helpt `target="_blank"` juist.
:::

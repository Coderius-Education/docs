...
<CodeOrg slug="paginas-koppelen" />

## Wat ga je leren?

Een klikbare link maak je met de `<a>`-tag. Je ziet ook hoe je hem in een nieuw tabblad laat openen.
...
:::info
In de preview hieronder werken links: een link zonder `target` opent in het voorbeeldvenster zelf, een link mét `target="_blank"` opent een echt nieuw tabblad. Heb je in het voorbeeld een link gevolgd, dan verschijnt erboven de knop **Terug naar je pagina**; je code staat nog in de editor. Sommige sites weigeren zich in een klein venster te laten tonen; dan blijft het voorbeeld leeg en helpt `target="_blank"` juist.
:::

### En knoppen?

Naast links zie je op websites vaak **knoppen** — die maak je met de `<button>` tag. Een knop alleen doet niets, je hebt JavaScript nodig om iets te laten gebeuren bij een klik:

```html
<button>Klik mij</button>
```

In de **JavaScript-cursus** leer je hoe je een knop interactief maakt. Voor nu: weet dat `<button>` bestaat en op een gewone knop lijkt.
...
- Wat gebeurt er als je `href` leeg laat (`href=""`), en wat als je er een `#` in zet (`href="#"`)?
...
Zonder `href` is de tekst niet klikbaar, en je ziet het meteen: de browser laat hem zwart en zonder streepje, precies als gewone tekst. Blauw en onderstreept is dus geen opmaak die bij de `<a>`-tag hoort, maar bij een `<a>` die ergens naartoe wijst. Zie je je link niet oplichten, dan ontbreekt `href`.

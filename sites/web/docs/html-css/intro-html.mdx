...
- **`<strong>`** — belangrijke (vetgedrukte) tekst. Net als `<b>`, maar `<strong>` vertelt screenreaders en zoekmachines dat het *belangrijk* is.
- **`<em>`** — benadrukte (cursieve) tekst. Net als `<i>`, maar `<em>` betekent semantisch "benadrukken".
- **`<br>`** — een **regeleinde**. Zelfsluitend (heeft geen `</br>`). Gebruik spaarzaam — meestal kun je beter een nieuwe `<p>` openen.
- **`<hr>`** — een **horizontale lijn** ter scheiding van secties. Ook zelfsluitend.
...
{/* stijl-uitzondering: uitroepteken citaat uit het codevoorbeeld erboven */}

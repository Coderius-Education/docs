...
const KnowledgeGraph = () => {
  const [selectedTutorialIndex, setSelectedTutorialIndex] = useState(
    knowledgeGraph.tutorials.length - 1,
  );
  ...
  const getCurrentLevels = (tutorialIdx) => {
    const levels = {};
    for (const c of knowledgeGraph.concepts) levels[c.id] = 0;
      ...
      >
        ...
        >
          <svg width="100%" height="100%" viewBox="0 0 750 500">
            <title>Kennisgraaf: verbanden tussen concepten</title>
            {/* Edges */}
            {knowledgeGraph.connections.map((conn) => (
              <line
                ...
                >
                  <circle
                    ...
                    style={{
                      ...
                      filter: isImpacted ? 'drop-shadow(0 0 8px var(--ifm-color-primary))' : 'none',
                    }}
                  />
                  <text
                    x={pos.x}
          ...
          {tutorial.impact.map((imp) => (
            ...
            >
              <strong style={{ display: 'block', marginBottom: '5px' }}>
                {knowledgeGraph.concepts.find((c) => c.id === imp.concept).label} (Niveau{' '}
                {imp.level})
              </strong>
              <span style={{ fontSize: '14px', color: 'var(--ifm-color-content-emphasis)' }}>
                {imp.depth}
              </span>
            </div>
          ))}

...
const SkillTree = () => {
  ...
  const getCurrentLevels = (tutorialIdx) => {
    const levels = {};
    for (const c of concepts) levels[c.id] = 0;
    for (let i = 0; i <= tutorialIdx; i++) {
      for (const imp of knowledgeGraph.tutorials[i].impact) {
  ...
  return (
    ...
    >
          ...
          {knowledgeGraph.tutorials.map((t, i) => (
            <button
              ...
              style={{
                ...
                border: '2px solid',
                borderColor:
                  i === selectedTutorialIndex
                    ? 'var(--ifm-color-primary)'
                    : 'var(--ifm-color-emphasis-300)',
                backgroundColor:
                  i === selectedTutorialIndex ? 'var(--ifm-color-primary)' : 'transparent',
              ...
              >
                ...
                >
                  {Array.from({ length: maxLevel + 1 }).map((_, i) => {
                    ...
                    return (
                      <div
                        // biome-ignore lint/suspicious/noArrayIndexKey: index is het level-nummer zelf, er is geen andere identiteit per item.
                        key={`level-${i}`}
                        style={{ position: 'relative' }}
                        title={
                          isImpacted
                            ? tutorial.impact.find(
                                (imp) => imp.concept === concept.id && imp.level === i,
                              )?.depth
                            : `Level ${i}`
                        }
                      >
                        ...
                        {isImpacted && (
                          ...
                          >
                            <strong>Nieuw:</strong>{' '}
                            {tutorial.impact.find((imp) => imp.concept === concept.id).depth}
                            <div
                              style={{
                                position: 'absolute',
                                top: '-5px',
      ...
      >
        <p style={{ margin: 0 }}>
          <strong>Tip:</strong> Klik op de tutorial-knoppen bovenaan om te zien hoe de lijnen tussen
          de levels worden ingevuld. Het oplichtende punt met de tekstballon laat zien wat je{' '}
          <em>precies</em> in die les hebt geleerd.
        </p>
      </div>
    </div>
  );

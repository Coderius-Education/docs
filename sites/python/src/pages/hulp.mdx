...
```python
# FOUT Geen inspringing
if leeftijd >= 18:
print("Volwassen")

# GOED Met inspringing
if leeftijd >= 18:
    print("Volwassen")
```
:::
...
print(leeftijd)     # FOUT NameError
leeftijd = 15

# Typfout in de naam (Hoofdletters zijn belangrijk!)
naam = "Alex"
print(Naam)         # FOUT NameError

# Vergeten aanhalingstekens bij tekst
print(Hallo)        # FOUT Python denkt nu dat Hallo een variabele is!
print("Hallo")      # GOED
```
:::
...
# FOUT TypeError: Python mag geen nummers en letters plussen
print("Ik ben " + leeftijd + " jaar oud.")

# GOED De oplossing is altijd een f-string:
...
# GOED Controleer eerst de invoer
if noemer != 0:
    print(10 / noemer)
```
:::
...
# FOUT IndexError (er is geen 4e persoon op index 3!)
print(namen[3])
...
# FOUT KeyError (want leeftijd ontbreekt in de dictionary)
print(leerling["leeftijd"])

# GOED Controleer eerst (bijvoorbeeld in een if statement)
...
```python
# FOUT Infinite loop: 'teller' is en blijft altijd kleiner dan 5!
teller = 0
while teller < 5:
    print("Deze regel print Python tot het einde der tijden...")

# GOED Vergeet teller += 1 niet aan de loop toe te voegen:
teller = 0
while teller < 5:
...
```python
# FOUT Dit slaat de waarde 18 op in de variabele 'leeftijd' (Fout in een If-statement!)
if leeftijd = 18:
    print("Bingo!")

# GOED Dit vergelijkt de waarde van leeftijd met 18
if leeftijd == 18:
    print("Bingo!")
```
:::
...
```python
# FOUT Fout: dit print het woord Error maar één keer aan het EINDE van de loop!
for i in range(5):
print("Error")

# GOED Goed: in de loop
for i in range(5):
    print("Success")
```
:::
...
# GOED Wil je 1 tot en met 5 printen? Dan stop je bij 6!
for i in range(1, 6):
    print(i)
```
:::
...
# FOUT Dit doet niks en leidt niet tot een error (zeer frustrerende bug!)
zeg_hallo

# GOED Functies eisen dat de parameters direct volgen tussen haakjes
zeg_hallo()
```
:::
...
```python
# FOUT Je kunt het resultaat hier niet opslaan om mee door te werken
def optellen(a, b):
    print(a + b)

x = optellen(3, 5)   # x heeft nu de waarde 'None'!

# GOED Met return kun je het resultaat gebruiken in je code:
def optellen(a, b):
    return a + b

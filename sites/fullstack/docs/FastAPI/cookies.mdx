...
`max_age` is de houdbaarheid in seconden. Hier is dat 30 dagen: `60 * 60 * 24 * 30`. Laat je `max_age` weg, dan verdwijnt de cookie zodra de bezoeker zijn browser afsluit.
...
Onthoud dat goed: een cookie staat op de computer van de bezoeker, en die kan er alles van maken wat hij wil. Voor een naam die je alleen als gemak invult is dat prima. Voor iets waar rechten aan hangen — wie mag verwijderen, wie is ingelogd — is het dat niet. Daar gaat de [volgende les](/docs/FastAPI/sessies) over.

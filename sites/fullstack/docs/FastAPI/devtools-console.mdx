...
Druk op F12 en kies **Console**. Zet in `static/js/app.js` een regel in de functie van de teller:
...
<DevtoolsPaneel tab="Console">
  <ConsoleRegels
    regels={[
      { tekst: 'nog 79 tekens', bron: 'app.js:3' },
      { tekst: 'nog 78 tekens', bron: 'app.js:3' },
      { tekst: 'Uncaught TypeError: Cannot read properties of null (reading \'addEventListener\')', bron: 'app.js:4', fout: true },
    ]}
  />
</DevtoolsPaneel>

Fouten staan er in het rood, met dezelfde verwijzing. Klik op `app.js:4` en de browser opent het bestand op die regel. De derde regel hierboven is wat je ziet als `defer` ontbreekt: het script draait voordat het formulier bestaat, en `veld` is dan `null`.

...
<Regel n={2}>
`FileResponse` vervangt `HTMLResponse`: je stuurt geen string meer terug, maar een bestand.
</Regel>

<Regel n={10}>
Het pad is dat van het bestand op je schijf, vanaf de map waarin `main.py` staat. FastAPI leest het bestand en stuurt de inhoud naar de browser. De HTML staat zo in een echt `.html`-bestand, met kleurtjes in je editor.
</Regel>

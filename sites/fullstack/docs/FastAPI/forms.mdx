...
<Voorkennis
  items={[
    {site: 'web', to: '/docs/html-css/formulieren', label: 'Formulieren'},
  ]}
/>
...
<Regel n={8} tot={10}>
Een gewone GET die het formulier toont. Het formulier is een HTML-bestand, dus `FileResponse`.
</Regel>

<Regel n={12}>
Het pad `/verstuur` is de `action` van het formulier, en `@app.post` de `method`. Die twee moeten kloppen met de HTML, anders komt het verzoek nergens aan.
</Regel>

<Regel n={13}>
`Form(...)` haalt het veld `naam` uit het verstuurde formulier. De parameter heet `naam` omdat het veld in de HTML `name="naam"` heeft; die twee namen moeten gelijk zijn.
</Regel>

<Regel n={14}>
De print komt in je terminal terecht, niet in de browser. Handig om te zien wat er binnenkomt.
</Regel>

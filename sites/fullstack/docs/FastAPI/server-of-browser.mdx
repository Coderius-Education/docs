...
<Voorkennis
  items={[
    {site: 'web', to: '/docs/js-basics/modern-dom', label: 'querySelector en querySelectorAll'},
  ]}
/>
...
De middelste kolom ken je nog niet. Tot nu toe betekende "naar de server" ook "de pagina verspringt": versturen, omleiding, alles opnieuw. Dat hoeft niet. In het volgende blok, [Zonder herladen met htmx](/docs/FastAPI/htmx), doet de server het werk terwijl de pagina blijft staan. De hoofdvraag verandert er niet door: moet iets bewaard blijven, dan gaat het naar de server, met of zonder herladen.
...
Je gastenboekveld heeft `maxlength="80"`. Open het in **Elementen**, zoals in [Console en Elementen](/docs/FastAPI/devtools-console): rechtermuisknop op het veld, Inspecteren, en dubbelklik op `maxlength="80"` om het weg te halen. Typ nu een heel lang bericht en verstuur het.

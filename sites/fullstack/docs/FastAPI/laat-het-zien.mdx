...
<CodeExercise>
{`# Waarom de teller een eigen bestand krijgt
from datetime import datetime

# Alles door elkaar: de teller staat tussen de berichten
rommelig = {
    "bezoeken": 12,
...
for sleutel, waarde in rommelig.items():
    print(f"{sleutel} -> {waarde}")

# Je template verwacht bij elke waarde een naam. De teller heeft die niet:
for sleutel, waarde in rommelig.items():
    naam = waarde["naam"] if isinstance(waarde, dict) else "GEEN NAAM"
    print(f"  {naam}")

# Netjes: twee opslagplaatsen, elk met één soort gegeven
gastenboek = {
    "bericht_100": {"naam": "Jan", "bericht": "Hallo"},
    "bericht_200": {"naam": "Sarah", "bericht": "Leuke site"}
}
...
print(f"\\n{len(gastenboek)} berichten, {bezoeken['aantal']} bezoeken")
for sleutel, bericht in gastenboek.items():

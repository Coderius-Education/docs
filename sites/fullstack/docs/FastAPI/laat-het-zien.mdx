...
Dit moet bewaard blijven en voor iedereen gelden, dus het hoort in je database. Lees de huidige stand met `db.get("bezoeken", 0)`, tel er één bij op, sla op, en stuur het getal mee naar je template.
...
async def berichten(request: Request):
    with SqliteDict("gastenboek.db") as db:
        bezoeken = db.get("bezoeken", 0) + 1
        db["bezoeken"] = bezoeken
        db.commit()
        alle_berichten = [v for k, v in db.items() if k != "bezoeken"]
...
Let op de regel die `bezoeken` uit de berichtenlijst filtert. Je gebruikt één database voor twee soorten gegevens, dus je moet ze uit elkaar houden. Dat is precies het soort rommeligheid waar je tegenaan loopt als alles in één key-value-opslag zit.
...
<CodeExercise>
{`# Twee soorten gegevens in één opslag uit elkaar houden
from datetime import datetime

database = {
    "bezoeken": 12,
    "bericht_100": {"naam": "Jan", "bericht": "Hallo"},
    "bericht_200": {"naam": "Sarah", "bericht": "Leuke site"}
}

# Alleen de berichten, niet de teller
berichten = [waarde for sleutel, waarde in database.items() if sleutel != "bezoeken"]
print(f"{len(berichten)} berichten, {database['bezoeken']} bezoeken")

for bericht in berichten:
    print(f"  {bericht['naam']}: {bericht['bericht']}")

# Een tijd bij een nieuw bericht
print("\\nNieuw bericht om", datetime.now().strftime("%H:%M"))`}
</CodeExercise>

...
<Regel n={13} tot={14}>
Het POST-endpoint uit [Eigen POST request](/docs/FastAPI/forms): `naam` komt uit het formulier.
</Regel>

<Regel n={15} tot={17}>
Nieuw is wat je ermee doet. Binnen het `with`-blok open je de database, zet je de naam onder de sleutel `laatste_naam` en schrijf je hem weg met `commit()`. Elke nieuwe naam overschrijft de vorige, want de sleutel is steeds dezelfde.
</Regel>

<Regel n={19}>
Het antwoord komt ná het `with`-blok: de database is dan al netjes gesloten.
</Regel>
...
<Regel n={4}>
`db.get(sleutel, standaard)` geeft de standaardwaarde terug als de sleutel nog niet bestaat, in plaats van te crashen. Zo werkt de pagina ook vóór de eerste keer opslaan.
</Regel>
...
Nee. Beide namen worden opgeslagen onder `"laatste_naam"`, dus de tweede overschrijft de eerste. Om meerdere namen op te slaan heb je **unieke keys** nodig, bijvoorbeeld met `time.time_ns()` (zie opdracht 5).
...
Maak een gastenboek met naam en bericht. Gebruik `time.time_ns()` als unieke key zodat berichten niet overschreven worden:

```python
import time

key = f"bericht_{time.time_ns()}"
db[key] = {"naam": naam, "bericht": bericht}
```

:::caution
Gebruik `time.time_ns()` (nanoseconden) en niet `int(time.time())` (hele seconden). Twee berichten die binnen dezelfde seconde binnenkomen krijgen anders dezelfde sleutel, en dan overschrijft het tweede het eerste.
:::

<details>
<summary>Tip</summary>

Je hebt twee `<input>` velden nodig (`name="naam"` en `name="bericht"`). Gebruik `time.time_ns()` om een unieke key te genereren, zodat elk bericht apart wordt opgeslagen.
...
Je berichten staan nu in de database, maar `/berichten` geeft ze nog als JSON terug. In [Een lijst tonen](/docs/FastAPI/lijst_tonen) zet je ze op een echte pagina.

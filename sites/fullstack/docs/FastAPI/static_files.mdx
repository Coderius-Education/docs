...
<Voorkennis
  items={[
    {site: 'web', to: '/docs/html-css/tekst-opmaken-css', label: 'CSS in een apart bestand koppelen'},
  ]}
/>
...
<Regel n={3}>
`StaticFiles` is het stuk van FastAPI dat bestanden ongewijzigd doorgeeft, zonder dat er een functie van jou aan te pas komt.
</Regel>

<Regel n={7}>
`app.mount` hangt de map `static` aan de URL `/static`. Alles in die map is daarmee bereikbaar in de browser: `static/css/style.css` staat op `/static/css/style.css`. De `name` heb je later nodig als je vanuit een template naar een bestand wilt verwijzen.
</Regel>
...
<Regel n={8}>
De `<link>` laadt de CSS uit het aparte bestand, via de URL die `app.mount` net heeft aangemaakt. De browser vraagt dat bestand zelf op, als tweede verzoek na de pagina. Dezelfde regel kun je op elke pagina zetten; dan delen ze één stylesheet.
</Regel>

...
<CodeExercise>
{`# Dezelfde teller, maar dan in Python
MAX = 80

bericht = "Hallo allemaal"
over = MAX - len(bericht)
print(f"{over} tekens over")

# Bij elke toetsaanslag opnieuw
tot_nu_toe = ""
for letter in "hallo":
    tot_nu_toe = tot_nu_toe + letter
    print(f"'{tot_nu_toe}' -> {MAX - len(tot_nu_toe)} tekens over")

# Het rekenwerk is hetzelfde. Het verschil zit hem in de plek: doe je dit in
# Python, dan moet elke letter eerst naar de server en het antwoord weer terug.`}
</CodeExercise>

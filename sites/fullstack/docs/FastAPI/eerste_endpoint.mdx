...
<Regel n={3}>
`FastAPI()` maakt een nieuwe app. Alles wat je server kan, hang je aan deze `app`.
</Regel>

<Regel n={5}>
De decorator `@app.get("/")` koppelt de functie eronder aan de URL `/`. Vraagt een browser die URL op, dan draait FastAPI deze functie.
</Regel>

<Regel n={6}>
De functie heet `root`, maar de naam doet er niet toe; de URL in de regel erboven bepaalt wanneer hij draait. Het `async` hoort bij FastAPI en mag je overnemen.
</Regel>

<Regel n={7}>
Wat de functie teruggeeft, krijgt de browser. Een dictionary zet FastAPI zelf om in JSON.
</Regel>
...
Je browser bewaart een kopie van elke pagina die hij ophaalt, de **cache**. Pas je iets aan en verandert er niets, dan kijk je naar die kopie. Druk op **F12**: dat opent de ontwikkelaarstools, een paneel naast of onder de pagina. Zolang dat paneel open staat, zit onder de rechtermuisknop op de herlaadknop van je browser de keuze **Cache legen en hard vernieuwen**. Sneller is **Ctrl+Shift+R**, dat doet hetzelfde zonder paneel.
:::
...
Start je server niet, met `NameError: name 'app' is not defined`? Dan staat je endpoint bóven de regel `app = FastAPI()` — Python leest van boven naar beneden. Bekijk de [troubleshooting pagina](/docs/troubleshooting) voor meer oplossingen.
:::
...
@app.get("/school")
async def school():
    return {"school": "Mijn school", "vak": "Informatica"}
```
...
```json
{"school": "Mijn school", "vak": "Informatica"}
```
